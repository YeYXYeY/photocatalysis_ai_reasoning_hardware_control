...
def main():
    ...
    logger = config_logger(
        runtime_paths["log_file"], logger_name="peptide_synthesis.server"
    )
    peptide_synthesis_server = PeptideSynthesisServer(
        *station_info["reactor_station"], logger, str(runtime_paths["task_dir"])
    )
    peptide_synthesis_server.start()
